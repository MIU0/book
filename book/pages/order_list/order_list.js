...
wx.switchTab({
    ...
    wx.request({
      ...
      success:function(res){
        ...
        if (res.data.code = 200) {
          ...
          console.log(order_content, 8079)
          var shopNull = 0
  ...
  loadingMore: function () {
    ...
    if (inforHasMore == 1) {
      ...
      var pa_order_content = that.data.order_content;
      var inforPage = that.data.inforPage;

      console.log(inforPage, 9999);
      //console.log(that.data.c_user_id, 8888);
      wx.request({
        url: `${getApp().globalData.baseUrl}/wechat/order/getCommAndTranOrderList`,
        data: {
        ...
        success: function (res) {
          ...
          if (res.data.code = 200) {
            var order_content = [];
            var new_order_content = pa_order_content.concat(res.data.data);
            that.setData({
              order_content: new_order_content
            });
            if (res.data.data.length < 10) {
              that.setData({
                inforHasMore: 0
              ...
              console.log(inforPage, 777);
            }

          } else {
            that.setData({
              order_content: pa_order_content,
              inforHasMore: 0,
            });
          }
